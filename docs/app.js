// Community Platform Competition Dashboard - Frontend
 ...
 * Returns { totals: { Alpha, Beta }, rosters: { Alpha: [], Beta: [] } }
 */
async function fetchTeamData() {
  try {
    const res = await fetch("https://www.community-platform.com/api/getsnowteams", { cache: "no-store" });
    ...
    for (const line of lines) {
      ...
      if (username) {
        if (team === "1") rosters.Alpha.push(username);
        else if (team === "0") rosters.Beta.push(username);
      }
    }

    // Sort rosters alphabetically
    rosters.Alpha.sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
    rosters.Beta.sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));

    return {
      totals: { Alpha: rosters.Alpha.length, Beta: rosters.Beta.length },
      rosters
    };
  } catch (err) {
 ...
 */
function renderTeamRosters(teamData, alphaFilter = "", betaFilter = "") {
  const alphaTable = document.getElementById("alphaRosterTable");
  const betaTable = document.getElementById("betaRosterTable");
  const alphaCount = document.getElementById("alphaRosterCount");
  const betaCount = document.getElementById("betaRosterCount");

  if (!teamData || !teamData.rosters) {
    // No data available (e.g., pre-reset mode)
    if (alphaTable) alphaTable.innerHTML = "<tr><td style='color:var(--text-muted);'>Team roster not available</td></tr>";
    if (betaTable) betaTable.innerHTML = "<tr><td style='color:var(--text-muted);'>Team roster not available</td></tr>";
    if (alphaCount) alphaCount.textContent = "";
    if (betaCount) betaCount.textContent = "";
    return;
  }
  ...
  // Filter rosters
  const alphaFilterLower = alphaFilter.toLowerCase();
  const betaFilterLower = betaFilter.toLowerCase();

  const filteredAlpha = alphaFilter
    ? rosters.Alpha.filter(u => u.toLowerCase().includes(alphaFilterLower))
    : rosters.Alpha;

  const filteredBeta = betaFilter
    ? rosters.Beta.filter(u => u.toLowerCase().includes(betaFilterLower))
    : rosters.Beta;

  // Update counts (show filtered/total)
  if (alphaCount) {
    alphaCount.textContent = alphaFilter
      ? `(${filteredAlpha.length}/${rosters.Alpha.length})`
      : `(${rosters.Alpha.length})`;
  }
  if (betaCount) {
    betaCount.textContent = betaFilter
      ? `(${filteredBeta.length}/${rosters.Beta.length})`
      : `(${rosters.Beta.length})`;
  }
  ...
  if (alphaTable) {
    alphaTable.innerHTML = `
      ...
      ${filteredAlpha.length > 0
        ? filteredAlpha.map(u => `<tr><td>${escapeHtml(u)}</td></tr>`).join("")
        : `<tr><td style="color:var(--text-muted);">No members matching "${escapeHtml(alphaFilter)}"</td></tr>`
      }
    `;
  }
  ...
  if (betaTable) {
    betaTable.innerHTML = `
      ...
      ${filteredBeta.length > 0
        ? filteredBeta.map(u => `<tr><td>${escapeHtml(u)}</td></tr>`).join("")
        : `<tr><td style="color:var(--text-muted);">No members matching "${escapeHtml(betaFilter)}"</td></tr>`
      }
    `;
  }
}
...
function computeTeamAdjustedPoints(events) {
  const totals = { Alpha: 0, Beta: 0, Unknown: 0 };
  ...
  for (const evt of events) {
    ...
    const team = evt?.attackerTeam === "Alpha" || evt?.attackerTeam === "Beta"
      ? evt.attackerTeam
      : "Unknown";
...
function applyAdjustedPointsToSummary(summary, events) {
  ...
  ["Alpha", "Beta", "Unknown"].forEach(team => {
...
/**
 * Fetch and process live data directly from Community Platform APIs
 * Returns { summary, users, events, roomsSummary, teamData }
 */
async function fetchLiveData() {
  // Fetch all data in parallel
  const [hitsResponse, teamsResponse, rooms] = await Promise.all([
    fetch("https://www.community-platform.com/api/gethits", { cache: "no-store" }),
    fetch("https://www.community-platform.com/api/getsnowteams", { cache: "no-store" }),
    loadRoomMapping()
  ]);
  ...
  for (const line of teamsLines) {
    ...
    if (username && team) {
      const teamName = team === "1" ? "Alpha" : "Beta";
  ...
  const teamStats = {
    Alpha: { users: 0, attacks: 0, hitsTaken: 0, adjustedPoints: adjustedPointsByTeam.Alpha || 0 },
    Beta: { users: 0, attacks: 0, hitsTaken: 0, adjustedPoints: adjustedPointsByTeam.Beta || 0 },
    Unknown: { users: 0, attacks: 0, hitsTaken: 0, adjustedPoints: adjustedPointsByTeam.Unknown || 0 }
  };
  users.forEach(u => {
    if (u.team === "Alpha" || u.team === "Beta") {
...
async function refreshFromAPI() {
  ...
  try {
    ...
    const alphaSearch = document.getElementById("alphaRosterSearch");
    const betaSearch = document.getElementById("betaRosterSearch");
    if (alphaSearch) alphaSearch.value = "";
    if (betaSearch) betaSearch.value = "";
...
const TEAM_COLORS = {
  Alpha: { bg: "rgba(59, 130, 246, 0.7)", border: "#3b82f6" },
  Beta: { bg: "rgba(239, 68, 68, 0.7)", border: "#ef4444" }
};

const TREND_COLORS = {
  overall: { border: "#111827", dash: [5, 5] },
  alpha: { border: "#1d4ed8", dash: [3, 3] },
  beta: { border: "#b91c1c", dash: [3, 3] }
};
...
async function renderTeamStats(summary, teamTotals) {
  ...
  const items = [
    { team: "alpha", label: "Alpha", data: ts.Alpha || {} },
    { team: "beta", label: "Beta", data: ts.Beta || {} }
  ];
...
function createScatter(ctx, summary) {
  ...
  const byTeam = {
    Alpha: pts.filter(p => p.team === "Alpha"),
    Beta: pts.filter(p => p.team === "Beta")
  };

  // Convert to x/y points for OLS
  const allPoints = pts.map(p => ({ x: p.attacks, y: p.hitsTaken }));
  const alphaPoints = byTeam.Alpha.map(p => ({ x: p.attacks, y: p.hitsTaken }));
  const betaPoints = byTeam.Beta.map(p => ({ x: p.attacks, y: p.hitsTaken }));

  // Compute OLS regression on the fly
  const overallOLS = computeOLS(allPoints);
  const alphaOLS = computeOLS(alphaPoints);
  const betaOLS = computeOLS(betaPoints);

  // Add labels for tooltip display
  if (overallOLS) overallOLS.label = "All Users";
  if (alphaOLS) alphaOLS.label = "Alpha";
  if (betaOLS) betaOLS.label = "Beta";
  ...
  const datasets = [
    {
      label: "Alpha",
      data: byTeam.Alpha.map(p => ({ x: p.attacks, y: p.hitsTaken, user: p.user })),
      backgroundColor: TEAM_COLORS.Alpha.bg,
      borderColor: TEAM_COLORS.Alpha.border,
      pointRadius: 5,
      pointHoverRadius: 8
    },
    {
      label: "Beta",
      data: byTeam.Beta.map(p => ({ x: p.attacks, y: p.hitsTaken, user: p.user })),
      backgroundColor: TEAM_COLORS.Beta.bg,
      borderColor: TEAM_COLORS.Beta.border,
      pointRadius: 5,
      pointHoverRadius: 8
    }
  ];
  ...
  const pengLine = computeLineDataset(state.regressionData.alpha, TREND_COLORS.alpha, "alpha");
  const reinLine = computeLineDataset(state.regressionData.beta, TREND_COLORS.beta, "beta");
...
function createDailyChart(ctx, events) {
  ...
  events.forEach(e => {
    ...
    if (!dailyData[date]) {
      dailyData[date] = {
        Alpha: { total: 0, attackers: {} },
        Beta: { total: 0, attackers: {} }
      };
    }
  ...
  const alphaData = last7.map(d => dailyData[d]?.Alpha?.total || 0);
  const betaData = last7.map(d => dailyData[d]?.Beta?.total || 0);
  ...
  const topAttackersPerDay = last7.map(d => {
    ...
    return {
      Alpha: getTop10(day?.Alpha),
      Beta: getTop10(day?.Beta)
    };
  });
...
/**
 * Check if debug mode is enabled via URL parameter (for suspicious detection)
 */
...
async function detectTraitors() {
  try {
    ...
    // Check Alpha roster for former Beta
    teamData.rosters.Alpha.forEach(user => {
      const preReset = preResetTeams[user.toLowerCase()];
      if (preReset && preReset.team === "Beta") {
        traitors.push({
          user: user,
          oldTeam: "Beta",
          newTeam: "Alpha"
    ...
    // Check Beta roster for former Alpha
    teamData.rosters.Beta.forEach(user => {
      const preReset = preResetTeams[user.toLowerCase()];
      if (preReset && preReset.team === "Alpha") {
        traitors.push({
          user: user,
          oldTeam: "Alpha",
          newTeam: "Beta"
...
async function renderTraitors() {
  ...
  // Group by direction
  const toBeta = traitors.filter(t => t.newTeam === "Beta");
  const toAlpha = traitors.filter(t => t.newTeam === "Alpha");

  const rows = traitors.map(t => {
    const arrow = t.oldTeam === "Alpha"
      ? `<span class="pill alpha">Alpha</span> → <span class="pill beta">Beta</span>`
      : `<span class="pill beta">Beta</span> → <span class="pill alpha">Alpha</span>`;
  ...
  container.innerHTML = `
    <div class="traitors-summary">
      <span class="traitor-stat">
        <span class="pill beta">Beta</span> → <span class="pill alpha">Alpha</span>: <strong>${toAlpha.length}</strong>
      </span>
      <span class="traitor-stat">
        <span class="pill alpha">Alpha</span> → <span class="pill beta">Beta</span>: <strong>${toBeta.length}</strong>
      </span>
    </div>
    <table>
      <tr>
...
/**
 * Initialize admin and debug panel visibility
 */
function initAdminPanel() {
  // Debug section (suspicious detection only)
...
function downloadEventsCSV() {
  ...
  link.href = url;
  link.download = `competition-events-${new Date().toISOString().split("T")[0]}.csv`;
  link.click();
...
function setupEventListeners() {
  ...
  if (EVENT_ENDED && refreshBtn) {
    ...
    refreshBtn.title = "The 2026 Seasonal Competition has ended. Data is now archived.";
    refreshBtn.style.opacity = "0.5";
    refreshBtn.style.cursor = "not-allowed";
  }
  refreshBtn?.addEventListener("click", () => {
    if (EVENT_ENDED) {
      alert("The 2026 Seasonal Competition has ended! The data shown is the final archived results.");
      return;
    }
  ...
  const alphaRosterSearch = document.getElementById("alphaRosterSearch");
  const betaRosterSearch = document.getElementById("betaRosterSearch");

  alphaRosterSearch?.addEventListener("input", () => {
    const alphaFilter = alphaRosterSearch.value || "";
    const betaFilter = betaRosterSearch?.value || "";
    renderTeamRosters(currentTeamData, alphaFilter, betaFilter);
  });

  betaRosterSearch?.addEventListener("input", () => {
    const alphaFilter = alphaRosterSearch?.value || "";
    const betaFilter = betaRosterSearch.value || "";
    renderTeamRosters(currentTeamData, alphaFilter, betaFilter);
  });
